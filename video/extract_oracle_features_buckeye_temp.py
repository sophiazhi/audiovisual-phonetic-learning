...
def phone2feat(phone_label, post_onset_time):
    ...
    # onehot = torch.normal(mean=torch.zeros((len(phones)+1,)) + 80, std=torch.zeros((len(phones)+1,))+20)
    # onehot[idx] = 300
    # return onehot

    # be1375fy_oracle_scalar_brokenstops20
    scalar = torch.Tensor([idx])
    if (phone_label == 'P' or phone_label == 'B') and post_onset_time < 20:
        scalar = torch.Tensor([len(phones)+1])
    elif phone_label in STOPS and post_onset_time < 20:
        scalar = torch.Tensor([len(phones)+2])
    return scalar
...
def process_videos(
    ...
    for video_name_idx, video_name in enumerate(tqdm(train_list)):
        ...
        for a_idx in range(num_audio_feature_frames):
            ...
            oracle_video_full = torch.empty((1,))
